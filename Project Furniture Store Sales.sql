...
SELECT avg(price) FROM sales WHERE price > 1200;

SELECT max(price) FROM sales;

SELECT min(price) FROM sales;

SELECT sum(price), country FROM sales group by country HAVING sum(price) > 1000;

SELECT avg(price), product, payment_type, name, city, country, account_created, last_login, latitude, longitude FROM sales group by product, payment_type, name, city, country, account_created, last_login, latitude, longitude HAVING avg(price) > 5000;

SELECT avg(price), product, country FROM sales group by product, country HAVING avg(price) > 2000;

SELECT max(price), product, country FROM sales group by product, country HAVING max(price) > 2000;

SELECT min(price), product, city FROM sales group by product, city HAVING min(price) < 3000;

SELECT * FROM sales WHERE payment_type = 'Mastercard' AND product = 'chair';

SELECT * FROM sales WHERE price = 1200 AND (country = 'United Kingdom' OR payment_type = 'Visa');

SELECT product, max(price), city, transaction_date FROM sales WHERE country = 'United States' group by product, city, transaction_date HAVING max(price) > 1500 ORDER BY transaction_date ASC;

SELECT * FROM sales WHERE price = 1200 AND (country LIKE 'United%' AND payment_type = 'Visa');